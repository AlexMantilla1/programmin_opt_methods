...
def sgd_momentum_nesterov(
    ...
    while (grad_magnitude > epsilon) and (iterations < max_iter):
        ...
        for k in np.arange(num_batches):
            ...
            # 2. update model
            model = model + alpha*velocity  
            # 3. approximate gradient of loss_function at model
    ...
    return {"value": list(model), "iterations": iterations}
...
def main() -> None:
    ...
    solution: Dict = sgd_momentum_nesterov(
        0.001, initial_model, loss_function
    )
    sol = solution["value"]
